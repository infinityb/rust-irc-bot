...
//     pub fn new(conf: &BotConfiguration) -> IoResult<BotConnection> {
//         let (host, port) = conf.server.clone();
//         let (conn, event_rx) = IrcConnection::new(host, port);
...
impl IrcConnection {
    pub fn new(host: &str, port: u16) -> IoResult<(IrcConnection, Receiver<IrcEvent>)> {
        ...
        TaskBuilder::new().named("core-dispatch").spawn(proc() {
            ...
            loop {
                println!("core-dispatch/LOOP WAIT");
                select! {
                    command = command_queue_rx.recv() => {
                        println!("core-dispatch/got command: {}", command);
                        match command {
                            RawWrite(value) => {
                    ...
                    string = raw_reader_rx.recv() => {
                        println!("core-dispatch/got string: {}", string);
