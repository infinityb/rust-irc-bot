#![deny(warnings)]
#![feature(slice_patterns, slice_concat_ext, conservative_impl_trait)]
#![feature(plugin)]
#![plugin(phf_macros)]
...
fn parse_appconfig() -> Option<BotConfig> {
    ...
    let filename = match args.as_slice() {
        ...
        &[_, ref filename, _..] => filename,
    };

    let mut buf = String::new();
    let read_result = File::open(filename).and_then(|mut f| f.read_to_string(&mut buf));
